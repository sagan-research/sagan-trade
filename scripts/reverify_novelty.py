...
def run_benchmark():
    ...
    # Predictions
    pred_sagan = tf.nn.sigmoid(model_sagan.predict(X_val_scaled, verbose=0)['logit']).numpy().flatten()
    pred_base = tf.nn.sigmoid(model_base.predict(X_val_scaled, verbose=0)['logit']).numpy().flatten()
    
    actual = y_val[:, 0]
    dm_stat, p_val = dm_test(actual, pred_base, pred_sagan)
    ...
    returns = main_prices.pct_change().dropna()
    val_indices = returns.index[split + window - 1 : split + window - 1 + len(y_val)]
    ...
    if high_dev_mask.sum() > 5:
        target = np.abs(y_ret[split:split+len(y_val)]) # Realized return magnitude
    ...
    confidences = tf.nn.sigmoid(all_preds['logit']).numpy().flatten()
    ...
    # 6. Inference Latency (Test 5)
    logger.info("Test 5: Inference Latency...")
    ...
    prob_buy = tf.nn.sigmoid(pred_stress['logit']).numpy().item()
    
    # Sagan config override check
    config.xai_confidence_threshold = 0.6
    # Neutrality defined on binary head: close to 0.5
    is_neutral = abs(prob_buy - 0.5) < 0.1
    
    results['stress_is_neutral'] = is_neutral
    results['stress_prob_buy'] = prob_buy
    logger.info(f"Stress Test: Prob Buy={prob_buy:.4f}, Is Neutral/Override={is_neutral}")
