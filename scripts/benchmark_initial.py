...
def train_tft_initial(prices, window=15, horizon=5, threshold=0.01, epochs=10):
    ...
    X_train = scaler.fit_transform(X_train.reshape(-1, n_stocks)).reshape(X_train.shape)
    ...
    return model, symbols, scaler

def run_synchronized_backtest(tickers, sym_model_ids, tft_model, tft_scaler, years=3, window=15, commission=0.0005):
    ...
    X_tft = np.array(X_tft, dtype=np.float32)
    X_tft = tft_scaler.transform(X_tft.reshape(-1, len(tickers))).reshape(X_tft.shape)
    ...
    for i in range(len(test_dates) - 1):
        ...
        bench_returns.append(next_rets.mean())
...
def get_latest_model_for_ticker(ticker):
    ...
    mask = (df['tickers'].apply(lambda x: isinstance(x, (list, str)) and ticker in x)) & (df['is_symbolic'] == True)
    filtered = df[mask]
...
def main():
    ...
    tft_model, _, tft_scaler = train_tft_initial(prices, epochs=15)
    ...
    s_ret, t_ret, b_ret = run_synchronized_backtest(tickers, sym_model_ids, tft_model, tft_scaler)
