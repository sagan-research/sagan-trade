...
The **Sagan Framework** introduces a paradigm shift: **Post-Prediction Explainability (XAI)** through Predictive Foundation Models.
...
Sagan operates on the principle that the model should first optimize for the most robust mathematical fit (the Alpha), and *then* provide a human-readable justification. By using **Predictive Regression** (Schmidt & Lipson, 2009), the "model" is not a weight matrix, but a **mathematical formula** (e.g., $Close \times \log(Volume)$). 

## 3. Technical Workings of the Sagan Engine

### 3.1 Step 1: Predictive Basis Discovery
...
Justification is provided by **SaganLLM**, a proprietary orchestration of Google’s open-weights **Gemma** models. The LLM take the Predictive formula and current market data to generate a narrative audit trail for every signal.

## 4. Academic Foundations
Sagan is built upon decades of research in Predictive discovery and evolutionary systems:
...
## 5. Conclusion
The Sagan Framework proves that transparency does not require a sacrifice in performance. By combining Predictive foundations with LLM-driven post-facto explanation, we create a system that is mathematically rigorous and humanly intuitive.
