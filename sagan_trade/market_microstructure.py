...
def estimate_hawkes(events, T):
    ...
    res = minimize(hawkes_log_likelihood, x0, args=(events, T),
                   method='L-BFGS-B', bounds=bounds, constraints=cons)
    mu, alpha, beta = res.x
...
# ----------------------------------------------------------------------
def simulate_price_range(ticker,
                         N=1_000_000,
                         n_bootstrap=10_000,
...
def analyze_portfolio(tickers, quick_mode=True):
    ...
    for ticker in tickers:
        try:
            ...
            res = simulate_price_range(
                ticker,
                N=500_000,
                n_bootstrap=n_boot,
                bootstrap_size=30_000
            )
...
def visualize_stock_insights(ticker):
    ...
    ax1.axvline(res['current_price'], color='red', linestyle='--', label=f'Current: ${res["current_price"]:.2f}')
    ax1.axvline(res['price_range'][0], color='green', linestyle=':', label=f'95% Lower: ${res["price_range"][0]:.2f}')
    ax1.axvline(res['price_range'][1], color='green', linestyle=':', label=f'95% Upper: ${res["price_range"][1]:.2f}')
    ax1.set_title('Bootstrapped Market-Clearing Prices')
    ax1.legend()

    params = res['estimated_parameters']
    mu_A, sigma_A = 2.0, 0.8
    A_sample = truncnorm.rvs(a=(0.1 - mu_A) / sigma_A, b=np.inf, loc=mu_A, scale=sigma_A, size=10000)
    ax2 = axes[0, 1]
    ...
    Var_R = (sigma_i**2 + lambda_star*(params['sigma_J']**2 + params['mu_J']**2)) * params['T']
    ...
    ax4 = axes[1, 1]
    A_scatter = truncnorm.rvs(a=(0.1 - mu_A) / sigma_A, b=np.inf, loc=mu_A, scale=sigma_A, size=5000)
