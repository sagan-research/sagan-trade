...
class ExecutionResult:
    ...
    position_path: np.ndarray | None = None
    cash_path: np.ndarray | None = None
...
class AlmgrenChrissModel(BaseExecutionModel):
    ...
    def optimize(self) -> ExecutionResult:
        ...
        if kappa > 0:
            ...
            for k in range(1, N + 1):
                ...
                x_k = (
                    X
                    * (np.exp(kappa * (T - t_prev)) - np.exp(kappa * (T - t_k)))
                    / (np.exp(kappa * T) - 1)
                )
                schedule[k - 1] = x_k
        else:
    ...
    def simulate(self, n_paths: int = 1000) -> dict[str, np.ndarray]:
        ...
        for k in range(1, N + 1):
            ...
            price_paths[:, k] = (
                price_paths[:, k - 1] + cfg.sigma * price_paths[:, k - 1] * dW + permanent_impact
            )
        ...
        for k in range(N - 1, -1, -1):
            for i, q in enumerate(state_grid):
                ...
                for t in np.linspace(min_trade, max_trade, n_trades):
                    ...
                    timing_risk = (
                        cfg.risk_aversion * cfg.sigma**2 * q_next**2 * (cfg.time_horizon / N)
                    )
...
class GatheralSchiedModel(BaseExecutionModel):
    ...
    def optimize(self) -> ExecutionResult:
        ...
        # Constraint: sum(x) = X
        ones = np.ones(N)
        KKT = np.block([[H_total, ones.reshape(-1, 1)], [ones.reshape(1, -1), np.zeros((1, 1))]])
    ...
    def simulate(self, n_paths: int = 1000) -> dict[str, np.ndarray]:
        ...
        for k in range(1, N + 1):
            ...
            impact_state = (
                impact_state * np.exp(-cfg.transient_impact_decay * dt) + cfg.permanent_impact * x_k
            )
            price_paths[:, k] = (
                price_paths[:, k - 1]
                + cfg.sigma * price_paths[:, k - 1] * dW
...
class VWAPModel(BaseExecutionModel):
    ...
    def optimize(self, volume_profile: np.ndarray | None = None) -> ExecutionResult:
        cfg = self.config
        N = cfg.num_intervals
        X = cfg.total_quantity
        T = cfg.time_horizon
...
class POVModel(BaseExecutionModel):
    ...
    def optimize(self, volume_forecast: np.ndarray | None = None) -> ExecutionResult:
        cfg = self.config
        N = cfg.num_intervals
        X = cfg.total_quantity
        T = cfg.time_horizon
...
class ImplementationShortfallModel(BaseExecutionModel):
    ...
    def optimize(
        self, alpha_signal: np.ndarray | None = None, urgency: float = 1.0
    ) -> ExecutionResult:
...
def create_execution_model(
    model_type: ExecutionModel, config: ExecutionConfig
) -> BaseExecutionModel:
...
def compare_execution_models(
    config: ExecutionConfig, models: list[ExecutionModel] | None = None, n_simulations: int = 1000
) -> pd.DataFrame:
