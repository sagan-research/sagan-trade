...
class DataLoader:
    ...
    def _load_yfinance(self, tickers: List[str], config: DataConfig) -> pd.DataFrame:
        ...
        if len(all_data) == 1:
            ...
            return pd.concat(all_data, keys=[t for t in tickers[:len(all_data)]], axis=1)
    ...
    def _load_csv(self, tickers: List[str], config: DataConfig) -> pd.DataFrame:
        ...
        if len(tickers) == 1 and os.path.isfile(config.csv_path):
            df = pd.read_csv(
                config.csv_path,
        ...
        for ticker in tickers:
            ...
            if os.path.exists(path):
                df = pd.read_csv(path, index_col=config.csv_index_col, parse_dates=config.csv_parse_dates)
                df = df.rename(columns=config.column_map)
                df["ticker"] = ticker
                all_data.append(df)
