...
class FeatureConfig:
    ...
    # Feature selection
    selection_method: Literal["mutual_info", "f_regression", "lasso", "rf_importance", "none"] = (
        "mutual_info"
    )
...
class TechnicalIndicators:
    ...
    @staticmethod
    def bollinger_bands(
        series: pd.Series, window: int = 20, std_dev: float = 2
    ) -> tuple[pd.Series, pd.Series, pd.Series]:
        """Bollinger Bands."""
    ...
    def mfi(
        ...
        positive_flow = (
            money_flow.where(typical_price > typical_price.shift(1), 0).rolling(window).sum()
        )
        negative_flow = (
            money_flow.where(typical_price < typical_price.shift(1), 0).rolling(window).sum()
        )
    ...
    def keltner_channels(
        high: pd.Series, low: pd.Series, close: pd.Series, window: int = 20, atr_mult: float = 2
    ) -> tuple[pd.Series, pd.Series, pd.Series]:
        """Keltner Channels."""
...
class MicrostructureFeatures:
    ...
    def vpin(
        ...
        vpin_series = pd.Series(
            vpin_vals, index=cum_vol.iloc[:: len(cum_vol) // n_buckets].index[: len(vpin_vals)]
        )
        return vpin_series.reindex(total_volume.index).ffill()

    @staticmethod
    def realized_volatility(
        returns: pd.Series, window: int = 100, scaling: float = np.sqrt(252 * 390)
    ) -> pd.Series:
    ...
    def amihud_illiquidity(
        returns: pd.Series, volume: pd.Series, price: pd.Series, window: int = 100
    ) -> pd.Series:
...
class FeatureEngine:
    ...
    def fit(self, data: pd.DataFrame, target: pd.Series | None = None) -> FeatureEngine:
    ...
    def fit_transform(self, data: pd.DataFrame, target: pd.Series | None = None) -> pd.DataFrame:
    ...
    def generate_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.include_trend:
            for w in (
                self.config.short_windows + self.config.medium_windows + self.config.long_windows
            ):
        ...
        if self.config.include_momentum:
            for w in [14, 20]:
                ...
                features[f"williams_r_{w}"] = (
                    TechnicalIndicators.williams_r(high, low, close, w) / 100 + 1
                )
        ...
        if self.config.include_volatility:
            for w in [10, 20, 50]:
                ...
                features[f"hist_vol_{w}"] = features["log_returns_1"].rolling(w).std() * np.sqrt(
                    252
                )
        ...
        if self.config.include_volume:
            ...
            for w in [14, 20]:
                features[f"mfi_{w}"] = (
                    TechnicalIndicators.mfi(high, low, close, volume, w) / 100 - 0.5
                )
        ...
        if self.config.clip_outliers > 0:
            for col in features.columns:
                _zscore = (features[col] - features[col].mean()) / (features[col].std() + 1e-10)
    ...
    def _parabolic_sar(
        ...
        for i in range(2, len(close)):
            if trend.iloc[i - 1] == 1:  # Uptrend
                sar.iloc[i] = sar.iloc[i - 1] + af_series.iloc[i - 1] * (
    ...
    def _yang_zhang_vol(
        self, open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, window: int = 20
    ) -> pd.Series:
        ...
        rs = (log_ho * (log_ho - log_co) + log_lo * (log_lo - log_co)).rolling(window).sum() / (
            window - 1
        )
    ...
    def select_features(
        self, features: pd.DataFrame, target: pd.Series, method: str | None = None
    ) -> list[str]:
        ...
        if method == "mutual_info":
            ...
            importance = pd.Series(np.abs(lasso.coef_), index=X.columns).sort_values(
                ascending=False
            )
...
def create_feature_engine(config: FeatureConfig | None = None, **kwargs) -> FeatureEngine:
...
def generate_features(data: pd.DataFrame, config: FeatureConfig | None = None) -> pd.DataFrame:
