...
class GatedResidualNetwork(nn.Module):
    ...
    def forward(self, x: Tensor, context: Optional[Tensor] = None) -> Tuple[Tensor, Optional[Tensor]]:
        """
        Args:
...
class VariableSelectionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.flatten = nn.Flatten(start_dim=-2)  # [..., num_inputs, input_size] -> [..., num_inputs * input_size]
...
class InterpretableMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        q = self.q_proj(query).view(*batch_shape, seq_len_q, self.num_heads, self.head_dim).transpose(-3, -2)
        k = self.k_proj(key).view(*batch_shape, seq_len_k, self.num_heads, self.head_dim).transpose(-3, -2)
        v = self.v_proj(value).view(*batch_shape, seq_len_k, self.num_heads, self.head_dim).transpose(-3, -2)
        ...
        # Reshape and project
        attn_output = attn_output.transpose(-3, -2).contiguous().view(
            *batch_shape, seq_len_q, self.hidden_size
        )
...
class TemporalFusionTransformer(nn.Module):
    ...
    def __init__(self, config: TFTConfig):
        ...
        self.output_projection = nn.Linear(
            config.hidden_size, 
            config.output_size * num_quantiles
        )
    ...
    def forward(
        ...
        (static_weights, 
         ...
         static_context_state_c) = self.static_encoders(static_inputs)
        ...
        enc_processed, _ = self.encoder_post_lstm_grn(enc_lstm_out, context=static_context_enrichment)
        ...
        # 7. Post-LSTM GRN
        dec_processed, _ = self.decoder_post_lstm_grn(dec_lstm_out, context=static_context_enrichment)
        ...
        enriched, _ = self.static_enrichment_grn(temporal_features, context=static_context_enrichment)
        
        # Split back
        enc_len = self.num_encoder_steps
        enc_enriched = enriched[:, :enc_len]
        ...
        output = self.output_projection(final_features)  # [batch, dec_len, output_size * num_quantiles]
