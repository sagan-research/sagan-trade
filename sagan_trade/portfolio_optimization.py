...
import numpy as np
import pandas as pd
from scipy import optimize, cluster, linalg
...
class OptimizationConfig:
    ...
    risk_aversion: Optional[float] = None  # If None, estimate from market
...
class OptimizationResult:
    ...
    effective_number_assets: Optional[float] = None
    
    # Black-Litterman specific
    posterior_mu: Optional[np.ndarray] = None
    posterior_sigma: Optional[np.ndarray] = None
    implied_returns: Optional[np.ndarray] = None
...
class HierarchicalRiskParity(BaseOptimizer):
    ...
    def _build_linkage(self, corr: np.ndarray) -> np.ndarray:
        ...
        linkage = cluster.hierarchy.linkage(
            dist, method=self.config.linkage_method
        )
        return linkage
...
class BlackLittermanOptimizer(BaseOptimizer):
    ...
    def optimize(
        ...
        view_returns: Optional[np.ndarray] = None,
        view_covariance: Optional[np.ndarray] = None,
        covariance: Optional[np.ndarray] = None,
        **kwargs
    ) -> OptimizationResult:
        """
        ...
        if views is not None and view_confidence is not None:
            ...
            P = pick_matrix
            Q = view_returns
            Omega = view_covariance if view_covariance is not None else self.config.tau * P @ sigma @ P.T
        else:
        ...
        **kwargs
        ...
        if self.objective == "max_sharpe":
            ...
            if prob.status in ["optimal", "optimal_inaccurate"]:
                ...
                weights = w_opt / np.sum(w_opt) if np.sum(w_opt) != 0 else np.ones(n_assets) / n_assets
            else:
            ...
            prob.solve(solver=self.config.solver, verbose=self.config.verbose)
            weights = w.value if prob.status in ["optimal", "optimal_inaccurate"] else np.ones(n_assets) / n_assets
