...
class AsymmetricRiskEngine:
    ...
    based on target volatility targets and maximum drawdown budgets.
    """
    def __init__(self, target_vol: float = 0.15, max_drawdown_limit: float = 0.075):
        self.target_vol = target_vol
        self.max_drawdown_limit = max_drawdown_limit

    def get_risk_multiplier(self, prices: pd.Series) -> pd.Series:
        """
        Calculates a dynamic position scaling multiplier [0, 1] based on
        rolling volatility and cumulative drawdown of the underlying price series.
        """
