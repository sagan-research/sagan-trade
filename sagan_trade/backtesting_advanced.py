        ...
        return pd.DataFrame(
            {
                ...
                "Value": [
                    f"{self.mean_return:.2%}",
                    f"{self.std_return:.2%}",
                    f"{self.mean_sharpe:.3f}",
                    f"{self.individual_results[0].sortino_ratio:.3f}"
                    if self.individual_results
                    else "N/A",
                    f"{self.individual_results[0].calmar_ratio:.3f}"
                    if self.individual_results
                    else "N/A",
                    f"{self.mean_max_dd:.2%}",
                    f"{np.mean([r.win_rate for r in self.individual_results]):.2%}"
                    if self.individual_results
                    else "N/A",
                    f"{np.mean([r.profit_factor for r in self.individual_results]):.3f}"
                    if self.individual_results
                    else "N/A",
                    f"{self.psr:.3f}",
                    f"{self.dsr:.3f}",
...
class WalkForwardBacktester(BaseBacktester):
    ...
    def __init__(self, config: BacktestConfig | None = None, expanding: bool = False):
    ...
    def run(
        ...
        **kwargs,
        ...
        for fold_id, (train_idx, test_idx) in enumerate(self.generate_splits(n_samples)):
            ...
            # Train strategy
            positions = strategy(train_prices, train_signals, test_prices, test_signals, **kwargs)

            # Evaluate on test period
            result = self._evaluate_fold(
                fold_id, train_idx, test_idx, prices.index, test_prices, positions
            )
    ...
    def _compute_metrics(self, result: BacktestResult, returns: pd.Series):
        ...
        sortino = (
            (excess.mean() / downside.std() * np.sqrt(ann_factor))
    ...
    def _aggregate_results(
        self, results: list[BacktestResult], method: str
    ) -> AggregatedBacktestResult:
        ...
        for combo in itertools.combinations(range(n_base), n_test_folds):
            ...
            if len(test_indices) > 0:
                ...
                train_indices = train_indices[
                    (train_indices < test_min - purge) | (train_indices > test_max + embargo)
                ]
...
class MonteCarloBacktester(BaseBacktester):
    ...
    def _compute_portfolio_returns(
        self, returns: pd.DataFrame, positions: pd.DataFrame
    ) -> pd.Series:
