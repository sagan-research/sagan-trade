...
class TestSymbolicRegressor:
    ...
    def test_train_predict(self):
        ...
        model_id = regressor.train(target=self.data['Close'], signals=['Close', 'RSI'], data=self.data)
...
class TestTemporalFusionTransformer:
    ...
    def test_model_creation(self):
        ...
        assert outputs["predictions"].shape == (batch_size, 6, 3, 1)  # [batch, dec_len, n_quantiles, output_size]
...
class TestFeatureEngineering:
    ...
    def setup_method(self):
        ...
        self.data = pd.DataFrame({
            ...
            'high': 100 + np.cumsum(np.random.randn(n) * 0.3) + np.abs(np.random.randn(n) * 0.2),
    ...
    def test_technical_indicators(self):
        ...
        macd, signal, hist = ti.macd(self.data['close'])
        upper, middle, lower = ti.bollinger_bands(self.data['close'])
        atr = ti.atr(self.data['high'], self.data['low'], self.data['close'])
...
class TestIntegration:
    ...
    def test_full_pipeline(self):
        ...
        engine = FeatureEngine(config)
        features = engine.fit_transform(prices['ASSET_0'].to_frame(name='close').join(
            pd.DataFrame({
        ...
        exec_config = ExecutionConfig(total_quantity=10000)
