...
class HighFrequencyBacktester:
    ...
    def __init__(
        self, latency_ticks: int = 2, inventory_limit: int = 150, inventory_penalty: float = 0.0005
    ):
    ...
    def run_backtest(
        self, df: pd.DataFrame, predictions: np.ndarray, ticker: str, sentiment_coef: float = 0.25
    ) -> dict:
        num_ticks = len(df)
        ...
        sentiment_vals = (
            df["sentiment"].values if "sentiment" in df.columns else np.zeros(num_ticks)
        )
        hawkes_intensity = (
            df["hawkes_intensity"].values
            if "hawkes_intensity" in df.columns
            else np.ones(num_ticks) * 0.5
        )
        ...
        sharpe = (
            (avg_ret / (std_ret + 1e-8)) * np.sqrt(252 * 5000) * 0.05 if len(returns) > 0 else 0.0
        )
    ...
    def run_vectorized_backtest(
        self, df: pd.DataFrame, predictions: np.ndarray, ticker: str
    ) -> dict:
        """
        ...
        port_vals = _fast_vectorized_pnl(
            bid_prices, ask_prices, predictions, self.latency_ticks, lot_size, fee_rate
        )
        ...
        sharpe = (
            (np.mean(returns) / (np.std(returns) + 1e-8)) * np.sqrt(252 * 5000)
            if len(returns) > 0
            else 0.0
        )
        ...
        return {"metrics": metrics, "portfolio_values": final_vals.tolist()}
