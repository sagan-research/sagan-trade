...
class HawkesLOBSimulator:
    ...
    def simulate_ticks(self, ticker: str, num_ticks: int = 2000) -> pd.DataFrame:
        ...
        for tk, p in self.stock_profiles.items():
            ...
            p.setdefault(
                "heston_theta", p.get("volatility_annual", 0.15) ** 2
            )  # Long term variance
        ...
        _sigma_step = (prof["volatility_annual"] / np.sqrt(252 * 5 * 3600)) * mid  # per tick vol
        ...
        last_mid = mid
        _last_bid_size = base_depth
        _last_ask_size = base_depth
        ...
        for i in range(num_ticks):
            ...
            trade_occurred = np.random.uniform(0, 1) < (
                current_intensity / (current_intensity + 5.0)
            )
            if trade_occurred:
            ...
            dv = (
                prof["heston_kappa"] * (prof["heston_theta"] - current_variance) * dt_year
                + prof["heston_sigma_v"] * np.sqrt(max(current_variance, 0)) * dW_v
            )
            ...
            dynamic_spread = (
                min_spread
                + (mean_spread - min_spread) * (current_intensity / (mu + 1.0))
                + spread_noise
            )
            # Align spread to tick increments
            dynamic_spread = np.maximum(
                min_spread, np.round(dynamic_spread / tick_size) * tick_size
            )
            ...
            if ticker == "MRF":
                ...
                current_bid_size = int(
                    base_depth * size_fluctuation * (1.0 + 0.1 * np.sin(i / 50.0))
                )
                current_ask_size = int(
                    base_depth * (2.0 - size_fluctuation) * (1.0 + 0.1 * np.cos(i / 50.0))
                )
            else:
            ...
            _last_bid_size = current_bid_size
            _last_ask_size = current_ask_size
        ...
        delta_v_bid = np.where(
            df["bid_price"] > df["prev_bid_price"],
            df["bid_size"],
            np.where(
                df["bid_price"] == df["prev_bid_price"], df["bid_size"] - df["prev_bid_size"], 0
            ),
        )

        delta_v_ask = np.where(
            df["ask_price"] < df["prev_ask_price"],
            df["ask_size"],
            np.where(
                df["ask_price"] == df["prev_ask_price"], df["ask_size"] - df["prev_ask_size"], 0
            ),
        )
        ...
        df.drop(
            columns=["prev_bid_price", "prev_ask_price", "prev_bid_size", "prev_ask_size"],
            inplace=True,
        )
        ...
        # 4. Imbalance Ratio
        df["depth_imbalance"] = (df["bid_size"] - df["ask_size"]) / (
