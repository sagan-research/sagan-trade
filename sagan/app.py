...
if page == "Symbolic Hub":
    ...
    if 'vars' in st.session_state:
        ...
        if st.button("Train Symbolic Model", type="primary"):
            with st.status("Solving Equations...") as status:
                ...
                reg = SymbolicRegressor([ticker], signals=selected_vars, profile=perf_mode.lower())
                meta = reg.train()
                mid = reg.save()
    ...
    if col1.button("Develop All Mathematical Foundations", type="primary"):
        with st.status("Massive Parallel Fitting...") as status:
            ...
            engine = PortfolioSymbolicEngine(portfolio, profile=perf_mode.lower())
    ...
    gating_mode = st.selectbox("Quantamental Gating Mode", ["strict", "loose", "balanced", "none"], index=2, 
                               help="strict: Binary filter. loose: Weight modulation. balanced: Stop-loss filter.")
    ...
    if st.button("Launch Autonomous Research", type="primary"):
        with st.status("Initializing Autonomous Researcher...") as status:
            ...
            results = researcher.run_full_pipeline(ticker_auto, gating_mode=gating_mode)
    ...
    if 'auto_results' in st.session_state:
        ...
        # Fundamental Bias Bar (The WHY)
        f_data = res.get("fundamental", {})
        f_score = f_data.get("score", 0.0)
        f_bias = f_data.get("bias", "Neutral")
        
        c1, c2, c3 = st.columns([1, 1, 2])
        c1.metric("Fundamental Bias", f_bias)
        c2.metric("Fundamental Score", f_score)
        
        risk_data = res.get("risk", {})
        risk_level = risk_data.get("risk", "Low")
        c3.warning(f"Execution Risk: {risk_level} — {risk_data.get('message', '')}")

        st.divider()
        
        tab1, tab2 = st.tabs(["💡 Positioning & Fundamentals", "📊 Technicals & Backtest"])
        ...
        with tab1:
            ...
            with col1:
                ...
                with st.expander("Deep Reasoning & Methodology", expanded=True):
                    st.markdown("### How and Why this decision was made")
                    st.write(res.get("reasoning", "No reasoning provided."))
                    st.caption("Narrative generated by Sagan XAI. Methodology: TCN-Parallel Symbolic Search.")
            with col2:
                st.subheader("Fundamental Drivers (The WHY)")
                reasons = f_data.get("reasons", ["No specific fundamental drivers identified."])
                for r in reasons:
                    st.write(f"✅ {r}")
                
                # Show key metrics in a small table
        ...
        with tab2:
            col1, col2 = st.columns([1, 1])
            with col1:
                st.subheader("Technical Signal (The WHEN)")
                st.code(res["formula"])
                st.write(f"**Variables:** {', '.join(res['signals'])}")
            with col2:
                st.subheader("Historical Performance")
                bt = res["backtest"]
                mc1, mc2 = st.columns(2)
                mc1.metric("Backtest Return", f"{bt['total_return']:.2%}")
                mc2.metric("Sharpe Ratio", f"{bt['sharpe']:.2f}")
