...
Metrics run_backtest(const std::vector<double>& closes, double annual_factor) {
    ...
    for (size_t t = 205; t < closes.size(); ++t) {
        ...
        // --- COMPONENT INJECTION ---
        double sma = 0, var = 0;
for(int i=0; i<200 && t-i>=0; i++) sma += closes[t-i];
sma /= 200.0;
for(int i=0; i<200 && t-i>=0; i++) var += pow(closes[t-i] - sma, 2);
double sd = sqrt(var / 200.0) + 1e-6;
