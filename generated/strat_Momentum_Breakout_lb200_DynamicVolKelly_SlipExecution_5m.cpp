...
Metrics run_backtest(const std::vector<double>& closes, double annual_factor) {
    ...
    for (size_t t = 205; t < closes.size(); ++t) {
        ...
        // --- COMPONENT INJECTION ---
        double high10 = closes[t], low10 = closes[t];
for(int i=1; i<200 && t-i>=0; i++) {
