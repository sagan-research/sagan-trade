...
schtasks /Create /TN "%TASK_NAME%" /TR "python \"%SCRIPT_PATH%\"" /SC DAILY /ST 22:00 /RL HIGHEST /F

if %errorLevel% == 0 (
    echo.
    echo [SUCCESS] The Infinite Trading Daemon has been registered successfully!
    echo It will now run completely independently every day starting at 10:00 PM.
) else (
    echo.
